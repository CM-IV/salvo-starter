...
pub static CFG: LazyLock<Configs> = LazyLock::new(self::Configs::init);
...
impl Configs {
    pub fn init() -> Self {
        ...
        match serde_yml::from_str(&cfg_contents) {
            Ok(c) => c,
            Err(e) => panic!("Failed to parse configuration file, error message:{}", e),
        }
    }
}
pub static CERT_KEY: LazyLock<CertKey> = LazyLock::new(get_cert_key);
